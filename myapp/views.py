...
from myapp.models import Files, Locations,MyPIN
...
def upload(request):
    if request.method == 'POST':
        ...
        s = gpd.GeoSeries([Polygon([(0, 0), (1, 1), (0, 1)]),LineString([(0, 0), (1, 1), (1, 0)]),Point(0, 0), ])
        print(s)
        shp=gpd.GeoDataFrame.from_file(file2)
        # shp=gp.GeoDataFrame.from_file(file2)
        # ax = geo.plot()
        print("Shape file ")
        print(shp)
        shp1=shp.dropna()
        print("Clean Shapefile ")
        print(shp1)
        shp2=shp.dropna()
        ...
        for i in range(len(shp2)):
            list.append(ulpin_g(shp2.iloc[i]['centroid'].y,shp2.iloc[i]['centroid'].x))
        shp2['ulpin']=list    
        print(shp2)
       
        # update=MyPIN.objects.bulk_create()
...
def ulpin(request):
    if request.method == 'POST':
        ...
        try:
            ulpin=MyPIN.objects.get(upin=ulpin2)
